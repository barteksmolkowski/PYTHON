    ...
    def BazadanychSprawdzOtworz(func):
        def wrapper(self, *args, **kwargs):
            try:
                if not getattr(self, "_tworzymy_nowy", False):
                    if not os.path.isfile(self.nazwa):
                        raise FileNotFoundException(
                            f"Plik bazy '{self.nazwa}' nie istnieje"
                        )
                ...
                if len(self.bazadanych) == 0 and not getattr(
                ...
                for klucz, wartosci in self.bazadanych.items():
                    ...
                    for wiersz, wartosc in wartosci.items():
                        ...
                        if not isinstance(wartosc, (int, float, str, bool)):
                            raise UnsupportedValueTypeException(
                                type(wartosc), klucz, wiersz
                            )
            ...
            try:
                ...
                for klucz, wartosci in self.bazadanych.items():
                    ...
                    for wiersz, wartosc in wartosci.items():
                        ...
                        if not isinstance(wartosc, (int, float, str, bool)):
                            raise UnsupportedValueTypeFromSaveException(
                                type(wartosc), klucz, wiersz
                            )
    ...
    def BazadanychsprawdzDodajPrzesten(func):
        def wrapper(self, coDodac, nazwa):
            if not hasattr(self, "bazadanych") or self.bazadanych is None:
                raise NoDatabaseProvidedDodajPrzestrzenException(
                    "self.bazadanych = None"
                )

            if not isinstance(self.bazadanych, dict):
                raise InvalidRowContainerTypeDodajPrzestrzenException(
                    type(self.bazadanych)
                )
            ...
            for klucz, wiersze in self.bazadanych.items():
                ...
                for wiersz_id, wartosc in wiersze.items():
                    ...
                    if wartosc is None:
                        raise InvalidAdditionElementNoneDodajPrzestrzenException(
                            f"Wartość None w wierszu {wiersz_id}"
                        )

                    if coDodac == "wiersz":
                        if wiersz_id in istniejące_wiersze:
                            raise AutoGeneratedRowIdConflictDodajPrzestrzenException(
                                wiersz_id
                            )
            ...
            if coDodac == "wiersz":
                if not getattr(self, "autoWiersz", True) and (
                    nazwa is None or nazwa == ""
                ):
    ...
    def BazadanychsprawdzUsun(func):
        def wrapper(self, coUsun: Literal["kolumna", "wiersz", "element"], nazwa):
            ...
            if coUsun not in ("kolumna", "wiersz", "element"):
                raise InvalidRemovalTypeUsunException(
                    f"Nieprawidłowy typ do usunięcia: {coUsun}"
                )
            ...
            if coUsun == "kolumna":
                ...
                kolumna_znaleziona = any(
                    isinstance(k, tuple)
                    and len(k) == 2
                    and (k[0] == kolumna or k[1] == kolumna)
