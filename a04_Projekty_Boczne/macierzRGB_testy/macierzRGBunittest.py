        ...
        def wrapper():
            try:
                ...
                if expected_exception and isinstance(e, expected_exception):
                    TEST_STATS["passed"] += 1
                    print(
                        f"Test {func.__name__} zaliczony (rzucił oczekiwany wyjątek)."
                    )
                else:
                    TEST_STATS["failed"] += 1
                    print(
                        f"Test {func.__name__} NIE zaliczony! Błąd: {type(e).__name__}: {e}"
                    )
            else:
                if expected_exception:
                    TEST_STATS["failed"] += 1
                    print(
                        f"Test {func.__name__} NIE zaliczony! Nie rzucono oczekiwanego wyjątku {expected_exception.__name__}."
                    )
                else:
                    TEST_STATS["passed"] += 1
...
def test_random_matrices_with_tuples():
    matrices = MatrixRGB.random_matrices_with_tuples(
        num_matrices=2, count=3, x=2, y=2, tuple_length=3, low=1, high=5
    )
...
if __name__ == "__main__":
    ...
    print(
        f"Testy zakończone. Zaliczone: {TEST_STATS['passed']}, Nie zaliczone: {TEST_STATS['failed']}"
    )
    print("=== KONIEC TESTÓW ===")
