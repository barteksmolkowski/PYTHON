...
class GlebokoscLowerValueException(Exception):
    def __init__(self, glebokosc):
        super().__init__(
            f"Wartość {glebokosc} dla zmiennej Głębokość jest mniejsza od 1."
        )
...
def sprawdzDane(func):
    def wrapper(sciezki, glebokosc, tekst):
        if glebokosc <= 0:
            raise GlebokoscLowerValueException(glebokosc)
        if not isinstance(sciezki, str):
            raise sciezkiNotIsStrException(sciezki)
