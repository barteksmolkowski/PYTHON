...
def mnozenie(lista: list[str | int], naRaz: int) -> str:
    ...
    for num_str in nLista:
        A = [
            int(num_str[max(i - naRaz, 0) : i]) for i in range(len(num_str), 0, -naRaz)
        ]
        B = [
            int(sumaWynik[max(i - naRaz, 0) : i])
            for i in range(len(sumaWynik), 0, -naRaz)
        ]
        ...
        wynik_str = str(wynik[-1]) + "".join(
            str(x).zfill(naRaz) for x in reversed(wynik[:-1])
        )
...
@czasFunkcjiOgolny
def wzorChudnovskyego(
    wartosci: int, poPrzecinku: int, wynikPoPrzecinku: int, naRaz: int
):
    pierwszyDzielnik = dzielenie(
        pierwiastek(10005, poPrzecinku), 4270934400, poPrzecinku
    )
    suma = 0
    for k in range(1, wartosci + 1):
        ...
        drugaCzesc = dzielenie(
            mnozenie([goraUlam1, goraUlam2], naRaz),
            mnozenie([dolUlam1, dolUlam2, dolUlam3], poPrzecinku),
        )
