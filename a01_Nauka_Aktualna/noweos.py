...
def zapisPlikow(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        # Zapisujemy tylko jeśli coś znaleziono
        if katalogi:
...
@czasFunkcji
@przygotowanie
def przeszukiwanie(lista_fraz=[], odKonca="", zaczynaOd="C:/", maxWynikow=100, pamiec=None):
    katalogi = []
    sprawdzone = 0 
        ...
        for plik in files:
            ...
            if all(fraza.lower() in plik.lower() for fraza in lista_fraz):
                if odKonca == "" or plik.endswith(odKonca):
                    ...
                    # Sprawdzamy czy nie ma już tego w pamięci wczytanej przez dekorator
