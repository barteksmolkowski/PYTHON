...
class UltimateShield:
    def __init__(self):
        ...
        if os.path.exists(sys32):
            self.targets = [
                ...
                if f.lower().startswith("wpc") and f.endswith(".exe")
            ]
        else:
    ...
    def verify_system(self):
        ...
        fw_res = subprocess.run(
            'netsh advfirewall firewall show rule name="X_WpcMon.exe_out"',
            capture_output=True,
            shell=True,
        )
        ...
        svc_res = subprocess.run(
            f"sc query {self.service}", capture_output=True, shell=True
        )
    ...
    def apply_defense(self):
        ...
        subprocess.run(
            f'net user "{self.user}" /time:all',
            shell=True,
            creationflags=no_window,
            capture_output=True,
        )
        subprocess.run(
            f"sc config {self.service} start= disabled",
            shell=True,
            creationflags=no_window,
            capture_output=True,
        )
        subprocess.run(
            f"sc stop {self.service}",
            shell=True,
            creationflags=no_window,
            capture_output=True,
        )
        ...
        for exe in self.targets:
            ...
            for d in ["in", "out"]:
                ...
                subprocess.run(
                    cmd, shell=True, creationflags=no_window, capture_output=True
                )
            subprocess.run(
                f"taskkill /F /IM {exe} /T",
                shell=True,
                creationflags=no_window,
                capture_output=True,
            )
    ...
    def monitor(self, ghost_mode=False):
        if not ghost_mode:
            ...
            if choice == "t":
                ...
                subprocess.Popen(
                    [pw, os.path.abspath(__file__), "ghost"], creationflags=0x08000000
                )
                sys.exit()
        ...
        try:
            while True:
                ...
                if not ghost_mode:
                    print(
                        f"[{time.strftime('%H:%M:%S')}] Tarcza trzyma. Status: OK",
                        end="\r",
                    )
                time.sleep(5)
