...
class TestThresholding:
    ...
    def test_adaptive_threshold(self, mock_mtx):
        ...
        with patch.object(Thresholding, '_generate_gaussian_kernel', return_value=mock_kernel):
            result = thresholding.adaptive_threshold(mock_mtx, block_size=5, c=2, auto_params=False)

            if result.shape != (10, 10):
                logger.error(f"Assertion failed: result shape {result.shape} != (10, 10)")
            ...
            if not np.all(np.isin(unique_values, [0, 255])):
                logger.error(f"Assertion failed: result contains values other than 0 and 255: {unique_values}")
            assert np.all(np.isin(unique_values, [0, 255]))

            total_sum = np.sum(result)
            if total_sum >= 100 * 255:
                logger.error("Assertion failed: thresholding did not create any black pixels")
