...
@runtime_checkable
class DataAugmentationABC(Protocol):
    def augment(self, M: TypeMatrix, repeats: int) -> List[TypeMatrix]: ...


@runtime_checkable
class GeometryAugmentationABC(Protocol):
    ...
    @overload
    def rotate_90(
        self, M: np.ndarray, is_right: Literal[True] = True
    ) -> np.ndarray: ...

    @overload
    def rotate_90(self, M: np.ndarray, is_right: Literal[False]) -> np.ndarray: ...
...
class NoiseAugmentationABC(ABC):
    @abstractmethod
    def gaussian_noise(self, M: np.ndarray, std: Optional[float] = None) -> np.ndarray:
        pass

    @abstractmethod
    def salt_and_pepper(
        self, M: np.ndarray, prob: Optional[float] = None
    ) -> np.ndarray:
        pass
...
class MorphologyAugmentationABC(ABC):
    @abstractmethod
    def dilate(self, M: np.ndarray, kernel_size: int, **kwargs: Any) -> np.ndarray:
        pass

    @abstractmethod
    def erode(
        self, M: np.ndarray, kernel_size: int, fill: int = 0, **kwargs: Any
    ) -> np.ndarray:
        pass
        ...
        **kwargs,
        ...
        if is_right is True:
            ...
            dx, dy = (
                max_dx_dy
                if max_dx_dy
                else (random.randint(-4, 4), random.randint(-4, 4))
            )
...
class MorphologyAugmentation(MorphologyAugmentationABC):
    def _apply_morphology(
        self, M: np.ndarray, kernel_size: int, op: Literal["min", "max"], pad_value: int
    ) -> np.ndarray:
        M = np.asanyarray(M).astype(np.uint8)
        pad_size = (kernel_size - 1) // 2

        padded = np.pad(
            M, pad_width=pad_size, mode="constant", constant_values=pad_value
        )
