...
class Normalization:
    def normalize(self, M: Mtx, old_r: Range = (0, 255), new_r: Range = (0, 1)) -> Mtx:
        ...
        if denominator == 0:
            self.logger.warning(
                f"[normalize] Zero denominator detected (old_r={old_r}). Returning constant matrix with value {new_r[0]}"
            )
            return np.zeros_like(M, dtype=np.float32) + new_r[0]

        self.logger.debug(
            f"[normalize] Scaling from {old_r} to {new_r} using denominator {denominator}"
        )
    ...
    def z_score_normalization(self, M: Mtx) -> Mtx:
        ...
        self.logger.debug(
            f"[z_score_normalization] Calculated stats: mean={mean_val:.4f}, std={std_val:.4f}"
        )
    ...
    def process(
        ...
        if use_z_score:
            self.logger.info(
                f"[process] Applying Z-Score normalization to matrix of shape {M_np.shape}"
            )
        ...
        self.logger.info(
            f"[process] Applying Min-Max normalization with old_range={o_r}, new_range={n_r}"
        )
