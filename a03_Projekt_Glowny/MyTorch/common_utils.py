...
from MyTorch import ClassType, MetricsDict, T, T_Val
...
def get_v(kwargs: dict[str, Any], key: str, default: T_Val) -> T_Val:
    val = kwargs.get(key, default)

    try:
        return cast(T_Val, type(default)(val))
    except (ValueError, TypeError):
        log = logging.getLogger("NeuralRecognizer")
        log.warning(
            f"[bold yellow]TYPE_MISMATCH[/] | Key: [cyan]'{key}'[/] | "
            f"Received: [red]'{val}'[/] ({type(val).__name__}) | "
            f"Falling back to default: [green]{default}[/]"
        )
        return default
